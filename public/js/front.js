...
//
	...
	// getRequest(`http://localhost:3010/user/admin/?color11=red&color2=blue`)
...
//
	table.addEventListener('dblclick',e => {
		...
		if (e.target.closest('[data-type="dir"]')) {
			...
			let idDir = elem.getAttribute('data-id');
			console.log(idDir)
			// getRequest(`http://localhost:3010/user/admin/?idDir=${idDir}`)
			getRequest('http://localhost:3010/user/admin/?idDir=uploads/admin/testFolder').then((data)=>{
				console.log(data)
			})
